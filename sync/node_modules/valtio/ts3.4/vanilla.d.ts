...
declare type Snapshot<T> = T extends AnyFunction ? T : T extends AsRef ? T : T extends Promise<any> ? Awaited<T> : { readonly [K in keyof T]: Snapshot2<T[K]> }; type Snapshot2<T> = T extends AnyFunction ? T : T extends AsRef ? T : T extends Promise<any> ? Awaited<T> : { readonly [K in keyof T]: T[K] };declare type _Snapshot<T> = T extends AnyFunction ? T : T extends AsRef ? T : T extends Promise<any> ? Awaited<T> : {
...
declare type HandlePromise = <P extends Promise<any>>(promise: P) => Awaited<P>;
declare type CreateSnapshot = <T extends object>(target: T, receiver: object, version: number, handlePromise?: HandlePromise) => T;
declare const PROXY_STATE: unique symbol;
...
export declare function subscribe<T extends object>(proxyObject: T, callback: (ops: Op[]) => void, notifyInSync?: boolean): () => boolean;
